      ...
      <div class="col-6">
        <form method="POST" action="{{ route('admin.recipe.update', $recipe->id) }}" enctype="multipart/form-data">
          ...
          <div class="mb-3">
            ...
            <input type="text" class="form-control" id="thumbnail" name="thumbnail" value="{{ $recipe->thumbnail }}" >
          </div>

          <div class="mb-3">
            <label for="description" class="form-label">Description</label>
            <textarea class="form-control" id="description" rows="5" name="description">{{ $recipe->description }}</textarea>
          </div>

          <div class="mb-3 ">
            <select class="form-control js-ingredients" multiple="multiple" name="ingredients[]">
              @foreach($recipe->ingredients as $ingredient)
                <option selected="selected">{{ $ingredient->ingredent_name }}</option>
              @endforeach
            </select>
          </div>
